...
void OLED_Mode(){
	OLED_NewFrame();
	OLED_PrintString(32, 12, "普通模式", &font16x16, OLED_COLOR_NORMAL);
	OLED_PrintString(32, 35, "快速模式", &font16x16, OLED_COLOR_NORMAL);
	OLED_ShowFrame();
}
...
void OLED_Show(){
	  ...
	  OLED_DrawImage(-7, 0, &dogImg, OLED_COLOR_NORMAL);

	  Limit(&now.happy);
	  sprintf(message,"快乐值:%d",now.happy);
	  OLED_PrintString(55,0 , message, &font12x12, OLED_COLOR_NORMAL);

	  Limit(&now.hungry);
	  sprintf(message,"饥饿值:%d",now.hungry);
	  OLED_PrintString(55,13 , message, &font12x12, OLED_COLOR_NORMAL);

	  Limit(&now.tired);
	  sprintf(message,"疲倦值:%d",now.tired);
	  OLED_PrintString(55,26 , message, &font12x12, OLED_COLOR_NORMAL);

	  sprintf(message,"温度:%.1f℃",now.temperature);
	  OLED_PrintString(55,39 , message, &font12x12, OLED_COLOR_NORMAL);

	  sprintf(message,"湿度:%.1f%%",now.humidity);
	  OLED_PrintString(55,51 , message, &font12x12, OLED_COLOR_NORMAL);
...
uint8_t TEMP_MAX=30.0f;
uint8_t TEMP_MIN=20.0f;

uint8_t HUMID_MAX=60.0f;
uint8_t HUMID_MIN=20.0f;
